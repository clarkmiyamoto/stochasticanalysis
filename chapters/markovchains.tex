...
In the previous lectures, we were concerned with the problem in generative models. You have access to examples $\{x_i\}_i \overset{iid}{\sim} \pi$, and you want to learn a model $\pi_\theta \approx \pi$ to generate new samples.\\
\\
...
\end{proof}
As the person running experiments, I simply want to specify a target distribution $\pi$ \& a proposal kernel $q$, and check which one works well. However not every transition kernel satisfies detailed balance for arbitrary $\pi$. Perhaps we can create a framework which always emits detailed balance.\\
\\
Consider choosing a new kernel $p(x' | x) = q(x' | x)\, \alpha(x' |x)$, where I'll soon specify $\alpha$. In the detailed balance equation
\begin{align}
	\pi(x) q(y|x) \alpha(y |x) = \pi(y) q(x | y) \alpha(x|y) \implies \alpha(y|x) = \frac{\pi(y) q(x|y)}{\pi(x) q(y | x)}
\end{align}
For this to be a probability, the ratio must be $\leq 1$. However, this could easily be the other case, so let's symmetrize
\begin{align}
...
\begin{algorithmic}[1]
\Require Target $\pi$, proposal $q(\cdot\mid x)$, initial $X_0$, steps $N$
\For{$t = 0, 1, \dots, N-1$}
...
\begin{fact}
	The Metropolis Hasting algorithm satisfies detailed balance.
\end{fact}
\begin{proof}
	The corresponding transition kernel is
	\begin{align}
		p(x|y) = q(x|y) \alpha(x|y) + \delta(x-y) R(y), \quad R(y) = \int q(x|y) (1- \alpha(x|y)) dxd
	\end{align}
	Either $\alpha(y |x) = 1$ or $\alpha(x|y) = 1$. Assume the former
	\begin{align}
		\pi(y)  p(x|y) & = \pi(y) [q(x|y) + \delta(x-y) R(y)]\\
		& = \pi(y) q(x|y) + \pi(x) \delta(x-y) R(x)\\
		& = \pi(x) q(y|x) \frac{\pi(y) q(x|y)}{\pi(x) q(y|x)} + \pi(x) \delta(x-y) R(x)\\
		& = \pi(x) [q(y|x) \alpha(y | x)  + \delta(x-y) R(x)]\\
		& = \pi(x) p(y|x)
	\end{align}
\end{proof}
...
\end{eg}
\begin{problem}
	The previous MALA algorithm was stated for single step. Consider a modification to the MALA algorithm for multiple steps. What is the new correction factor $p(x' | x)/p(x|x')$ in the Metropolis acceptance ratio?
\end{problem}
...
\paragraph{Reversibility and Spectral Theorem}
The proof above assumed $P$ is diagonalizable.

\begin{fact}
	[Detailed balance $\iff$ self-adjointness]
	Let $P$ be the transition matrix of a Markov chain with stationary
	distribution $\pi$, and suppose $(P,\pi)$ satisfies detailed balance
	$\pi(x) P(x,y) = \pi(y) P(y,x)$. Then $P$ is self-adjoint on the weighted
	space $L^2(\pi)$ equipped with the inner product
	\begin{align}
		\langle f, g \rangle_\pi = \sum_x \pi(x) f(x) g(x).
	\end{align}
	Consequently $P$ has real eigenvalues
	$1 = \lambda_1 > \lambda_2 \geq \dots \geq \lambda_d \geq -1$
	and a basis of eigenvectors orthonormal in $\langle \cdot, \cdot \rangle_\pi$.
\end{fact}
\begin{proof}
	Define $D = \mathrm{diag}(\pi)$. Detailed balance reads
	$D P = (D P)^T = P^T D$, i.e. $DP$ is symmetric. Form the similarity
	transform $S = D^{1/2} P D^{-1/2}$. Note it is symmetric
	\begin{align}
		S^T & = D^{-1/2} P^T D^{1/2}\\
		    & = D^{-1/2} (D P D^{-1}) D^{1/2} & P^T D = D P\\
		    & = D^{1/2} P D^{-1/2} = S,
	\end{align}
	By the spectral theorem it has real eigenvalues and an orthonormal
	eigenbasis. Since $P = D^{-1/2} S D^{1/2}$ is similar to $S$, it shares the same (real) eigenvalues, and its eigenvectors are the columns of
	$D^{-1/2}$ applied to those of $S$, which are precisely orthonormal under
	$\langle \cdot, \cdot \rangle_\pi$.
\end{proof}
Notice  
...
\begin{theorem}
	For a time-discrete \& reversible Markov chain $P$ with spectral gap $\gamma = 1 - \lambda_2$ (2nd largest eigenvalue of $P$). We converge 
	\begin{align}
		\|\pi_k - \pi\|_{\textsf{TV}} \leq  \frac{1}{2} \sqrt{\chi^2(\pi_0 \| \pi)} (1-\gamma)^k
	\end{align}
...
\begin{definition}
	[$f$-Divergence] Given a convex function $f: \mathbb R \to \mathbb R$ s.t. $f(1) = 0$, the $f$-Divergence is
	\begin{align}
...
\bibitem[LPW09]{LPWMixingTimes}
David A. Levin, Yuval Peres, and Elizabeth L. Wilmer. \emph{Markov Chains and Mixing Times}. \url{https://www.cs.cmu.edu/~15859n/RelatedWork/MarkovChains-MixingTimes.pdf}
